...
def api_get_server_identity():
    try:
        ...
        return json_problem(OperationProblem(gettext("Failed to read server name.")))
...
def api_update_server_identity():
    ...
    try:
        result = _get_services().server_identity_service.save_server_name(
            data.get("server_name", "")
        )
        return json_data(result.as_dict(), message=gettext("Server name saved."))
        ...
        return json_problem(
            OperationProblem(
                gettext("Failed to update server name."),
