...
class AlertsService:
    ...
    def get_alert(self, alert_id: int) -> AlertDetail:
        ...
        raise NotFoundProblem(
            gettext("Alert not found."),
            title=gettext("Alert not found"),
    ...
    def mark_alert_read(self, alert_id: int) -> None:
        if self._config_manager.mark_alert_read(alert_id):
            return None
        raise OperationProblem(gettext("Failed to mark alert as read."))

    def clear_alerts(self) -> None:
        if self._config_manager.clear_alerts():
            return None
        raise OperationProblem(gettext("Failed to clear alerts."))

    def mark_all_alerts_read(self) -> None:
        if self._config_manager.mark_all_alerts_read():
            return None
        raise OperationProblem(gettext("Failed to mark all alerts as read."))

    def get_email_config(self) -> EmailConfig:
        smtp_config = read_runtime_smtp_config(self._runtime)
        ...
        if email_address and "email_address" not in smtp_config:
            smtp_config["email_address"] = email_address
        if from_address and "from_address" not in smtp_config:
            smtp_config["from_address"] = from_address

        return EmailConfig(
            config=smtp_config,
            has_smtp_password=bool(smtp_config.get("smtp_password")),
        )
    ...
    def save_email_config(self, data: dict[str, Any]) -> None:
        ...
        if not all([email, from_address, smtp_server, smtp_port, smtp_username]):
            raise ValidationProblem(
                gettext("Email, from address, SMTP server, port, and username are required.")
            )
        smtp_port_text = str(smtp_port).strip()
        if not smtp_port_text.isdigit() or not 1 <= int(smtp_port_text) <= 65535:
            raise ValidationProblem(gettext("SMTP port must be between 1 and 65535."))

        if not smtp_password:
            smtp_password = read_runtime_smtp_config(self._runtime).get("smtp_password", "")
        if not smtp_password:
            raise ValidationProblem(gettext("SMTP password is required."))
        ...
        try:
            ...
            raise OperationProblem(gettext("Failed to write SMTP configuration.")) from exc
