...
"""
    ...
    def _write_layout_before_local_publish(self):
        return self.runtime.is_fake or self.privileged_actions is None

    def _prepare_share_write(self):
        if self._write_layout_before_local_publish():
    ...
    def publish_managed_shares(self, content: str):
        """Publish the SSS-owned Samba shares file in the current process.

        The helper calls this method directly. Web code in real mode should use
        `_publish_sss_shares_file()` so the root-only write stays allowlisted.
        """
        self._ensure_layout_for_share_write()
        self._commit_sss_shares_file(content)

    def _publish_sss_shares_file(self, content: str):
        if self.runtime.is_fake or self.privileged_actions is None:
            self._commit_sss_shares_file(content)
            return
        ...
        try:
            self.privileged_actions.run(
                ...
                {"shares_config": content},
            )
        except PrivilegedActionClientError as exc:
            raise SMBOperationError(str(exc)) from exc
    ...
    def create_managed_share(self, name, path, writable=True, comment="", valid_users=None):
        ...
        self._publish_sss_shares_file("".join(new_lines))
