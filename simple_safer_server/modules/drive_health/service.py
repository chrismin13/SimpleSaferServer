...
def build_drive_health_summary(
    ...
    *,
    ...
    if collect_hdsentinel and get_hdsentinel_settings(config_manager)["enabled"]:
        ...
        # and used by scheduled checks; this compact summary stays focused on
        # one snapshot so status changes remain predictable.
...
def build_drive_health_page_result(config_manager, system_utils, runtime=None):
    """Run the explicit Drive Health page probe and return render-ready data."""
    runtime = runtime or get_runtime()
    ...
    hdsentinel_drives = []
    if get_hdsentinel_settings(config_manager)["enabled"]:
        hdsentinel_snapshot = collect_hdsentinel_snapshot(
            config_manager,
            system_utils,
            runtime=runtime,
        ...
        hdsentinel_drives = mark_managed_storage_drive(
            hdsentinel_drives,
            config_manager,
            system_utils,
            runtime=runtime,
    ...
    if smart is not None:
        summary["detail"] = "SMART details were collected."
    if hdsentinel_snapshot is not None and hdsentinel_snapshot_has_health(hdsentinel_snapshot):
        health_pct = hdsentinel_snapshot.get("health_pct")
        summary["hdsentinel_health"] = health_pct
        summary["hdsentinel_performance"] = hdsentinel_snapshot.get("performance_pct")
        summary["status"] = hdsentinel_summary_status(health_pct)
    ...
    return {
        "smart": smart,
        "missing_attrs": missing_attrs or [],
        "error": error,
